...
const initializeMap = () => {
  // 지도 초기 센터를 삼성전기부산으로 설정
  map.value = new window.Tmapv2.Map("map_div", {
    center: new window.Tmapv2.LatLng(35.0936583, 128.8526144),
    width: "100%",
    height: "100%",
    zoom: 15,
    httpsMode: true,
...
const getCurrentLocation = async () => {
  try {
    ...
    alert("현재위치를 출발지로 설정하겠습니다!");
    
    map.value.setCenter(new window.Tmapv2.LatLng(defaultLocation.lat, defaultLocation.lng));
    map.value.setZoom(15);
    setLocationMarker(defaultLocation, 'start');
...
const searchLocationSuggestions = async (keyword) => {
  if (!tData.value || !keyword || keyword.length < 2) {
    return [];
  }
  ...
  try {
    ...
    const marker = new window.Tmapv2.Marker({
      ...
      iconHTML: `<div class='_t_marker' style="position:relative;">
                  <i class="fa-solid fa-location-dot" style="color: ${type === 'start' ? '#4D96FF' : '#FF6B6B'}; font-size: 38px;"></i>
                </div>`,
...
const calculateRoute = async (routeType = 'car') => {
  try {
    ...
    const startLatLng = new window.Tmapv2.LatLng(startCoords.value.lat, startCoords.value.lng);
    const endLatLng = new window.Tmapv2.LatLng(endCoords.value.lat, endCoords.value.lng);
    ...
    return new Promise((resolve) => {
      const callbacks = {
        onComplete: function(result) {
          if (!result || !result._responseData || !result._responseData.features) {
            alert('경로를 찾을 수 없습니다.');
            resolve(null);
            return;
          }
          ...
          const details = {
            ...
            taxiFare: routeType === 'car' ? resultData[0].properties.taxiFare : null
          };
          resolve(details);
        },
        onProgress: function() {},
        onError: function(error) {
          console.error(`${routeType} 경로 검색 중 오류가 발생했습니다:`, error);
...
const drawRoute = (resultData) => {
  ...
  resultData.forEach(feature => {
    if (feature.geometry.type === "LineString") {
      const sectionPoints = feature.geometry.coordinates.map(coord => 
      ...
      sectionPoints.forEach(point => positionBounds.extend(point));
    }
  });
...
const clearMap = () => {
  lineArr.value.forEach(line => line.setMap(null));
  lineArr.value = [];
  
  labelArr.value.forEach(label => label.setMap(null));
  labelArr.value = [];
};
