<template>
  <div class="season-plan-container min-h-screen bg-gradient-to-b from-[#433629] to-[#2a1f15]">
    ...
    <div class="text-center">
      <h1 class="season-title">{{ koreanSeason }} 여행</h1>
...
export default {
  ...
  created() {
    ...
    this.setKoreanSeason(season);
    this.fetchSeasonPlan(season);
  },
  methods: {
    setKoreanSeason(season) {
      const seasonMap = {
        spring: "봄",
        summer: "여름",
        autumn: "가을",
      ...
      this.koreanSeason = seasonMap[season];
    },
    async fetchSeasonPlan(season) {
      try {
        this.loading = true;
        const response = await api.get(`/api/attraction/ai/plan/season/${season}`);
