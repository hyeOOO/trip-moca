          ...
          >
            ...
            <div class="place-info">
              <div class="flex gap-2 mb-2">
                <span
                  v-if="item.contentTypeName"
                  class="px-2 py-1 rounded-lg text-xs text-white tag"
                  :style="{
                    backgroundColor: getContentTypeColor(item.contentTypeId),
                  }"
                >
...
const handleDrop = (event, targetDay, targetIndex) => {
 ...
 // 다른 날짜로의 이동이거나 장바구니에서의 추가
   ...
   if (draggedItem.value.type === "cart") {
     ...
     // 장바구니에서 아이템 제거
     const cartItemIndex = planStore.getCartItems.findIndex(
       item => item.attractionId === draggedItem.value.item.attractionId
     );
     if (cartItemIndex !== -1) {
       planStore.removeCartItem(cartItemIndex);
     }
...
const saveModifiedPlan = async () => {
  try {
    ...
    formData.append(
      "plan",
      new Blob([JSON.stringify(planUpdateRequest)], {
        type: "application/json",
      })
    );

    // 이미지가 있다면 추가 (있는 경우에만)
    if (planData.value.newImage) {
      // newImage는 새로 업로드된 이미지 파일을 저장하는 속성입니다
